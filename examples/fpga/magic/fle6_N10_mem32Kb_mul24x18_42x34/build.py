...
try:
    ...
    builder = ctx.build_logic_block("mul", 1, 2)
    inst = builder.instantiate(
            ctx.create_multiplier(mul_a_width, width_b = mul_b_width),
            "i_mul")
    builder.connect(builder.create_input("a", mul_a_width, Orientation.west, (0, 0)), inst.pins["a"])
    builder.connect(builder.create_input("b", mul_b_width, Orientation.west, (0, 1)), inst.pins["b"])
    builder.connect(inst.pins["x"][:(mul_a_width + mul_b_width) // 2],
            builder.create_output("xl", (mul_a_width + mul_b_width) // 2, Orientation.east, (0, 0)))
    builder.connect(inst.pins["x"][(mul_a_width + mul_b_width) // 2:],
            builder.create_output("xh", len(inst.pins["x"]) - (mul_a_width + mul_b_width) // 2, Orientation.east,
                (0, 1)))
    mul = builder.commit()
    ...
    multile = ctx.build_tile(mul).fill( (0.15, 0.25) ).auto_connect().commit()
    ...
    for x, y in product(range(builder.width), range(builder.height)):
        if x == 2:
            if y % 2 == 0:
                builder.instantiate(multile, (x, y))
        elif x == 6:
            if y % 2 == 0:

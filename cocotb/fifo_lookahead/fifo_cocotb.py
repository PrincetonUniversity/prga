import cocotb
from cocotb.triggers import Timer,RisingEdge,Edge,First,NextTimeStep,ReadOnly
...
def always_star(dut):
    while True:
        ...
        if dut.empty.value.binstr != 'x' and dut.rd.value.binstr != 'x':
            config.valid = (~int(dut.empty.value) & int(dut.rd.value))
...
def prga_fifo_test(dut):
    ...
    test_c= Clock(dut.test_clk,2)

    cocotb.fork(test_c.start(test_time//clock_period))

...
# ----------------------------------------------------------------------------
class AbstractConfigProtoSerializer(with_metaclass(ABCMeta, object)):
    """Abstract base class for config proto serializer.

    Args:
        f (file-like object): the output stream
        packet_size (:obj:`int`): number of bytes per packet

    """

    def __init__(self, f, packet_size = 4 * 1024 * 1024):
        self._f = f
        # signature
        self._f.write(pack('<8s', 'prgacfgm'.encode('ascii')))
    ...
    def _serialize(self, obj):
        ...
        """
        self._f.write(pack('<I', obj.ByteSize()))
